...
class IMU:
    def __init__(self, imu_raw_loc, vicon_loc, imu_params_loc, is_test=False):
        ...
        if is_test == False:
            ...
            self.vicon_slerp = Slerp(
                self.vicon_ts, R.from_matrix(self.reshape_vicon_rot())
            )
        ...
        if is_test == False:
            self.inital_gyro_orien = self.vicon_slerp(
                self.imu_ts[self.start_index]
            ).as_euler("xyz", degrees=False)
            self.inital_gyro_orien = np.reshape(self.inital_gyro_orien, (3, 1))
        else:
            self.inital_gyro_orien = np.zeros((3, 1))
    ...
    def get_gyro_orientation(self):
        ...
        self.gyro_rot[:, :, 0] = R.from_euler("xyz", self.gyro_rpy[:, 0]).as_matrix()
    ...
    def get_vicon_rot_sync(self):
        ...
        for i in range(len(self.imu_ts_sync)):
            self.vicon_rot_sync[:, :, i] = self.vicon_slerp(
                self.imu_ts_sync[i]
            ).as_matrix()
